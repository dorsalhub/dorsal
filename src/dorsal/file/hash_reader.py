...
class HashReader:
    ...
    def get(
        ...
        """
        requested_hashes = list(dict.fromkeys(hashes))
        ...
        if hashes_to_process:
            ...
            if file_hasher_algos:
                try:
                    ...
                    for hash_function, hash_val in calculated_hashes.items():
                        ...
                        if not skip_cache and hash_val is not None:
            ...
            if "QUICK" in hashes_to_process:
                ...
                hashes_to_process.discard("QUICK")

            for hash_func in hashes_to_process:
                results[hash_func] = None

        return {h: results.get(h) for h in requested_hashes if h in results}
