...
def test_push_cache_conflict(mock_exit_cli, tmp_path):
    ...
    mock_exit_cli.assert_called_with(
        code=ANY, message="Error: --use-cache and --skip-cache flags cannot be used together."
    )
...
def test_push_skip_overwrite_conflict(mock_exit_cli, tmp_path):
    ...
    assert result.exit_code != 0
    mock_exit_cli.assert_called_with(
        code=ANY, message="Error: --skip-cache and --overwrite-cache flags cannot be used together."
    )
