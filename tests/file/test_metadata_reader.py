...
class TestMetadataReaderGetOrCreateRecord:
    def test_cache_hit_with_hash(self, metadata_reader_base, fs, mocker):
        ...
        mocker.patch(
            "dorsal.file.validators.file_record.FileRecordStrict.model_validate_json", return_value=mock_record_instance
        )
    ...
    def test_cache_hit_shallow_record_when_hashes_not_needed(self, metadata_reader_base, fs, mocker):
        ...
        mock_record_instance = MagicMock()
        mocker.patch(
            "dorsal.file.validators.file_record.FileRecord.model_validate_json", return_value=mock_record_instance
        )
...
class TestMetadataReaderDirectoryCoverage:
    ...
    def test_generate_processed_records_skipped_duplicate(self, reader_for_coverage, temp_dir_with_files):
        reader = reader_for_coverage
        reader._ignore_duplicates = True
    ...
    def test_generate_processed_records_dorsal_error(self, reader_for_coverage, temp_dir_with_files):
        reader = reader_for_coverage
        f1 = str(temp_dir_with_files / "f1.txt")
        (temp_dir_with_files / "f1.txt").touch()
        ...
        records, path_map = reader.generate_processed_records_from_directory(dir_path=str(temp_dir_with_files))
